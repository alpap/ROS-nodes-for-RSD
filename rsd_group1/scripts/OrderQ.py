...
from rsd_group1.msg import Num, lego_brick, mes_order, Log, msg_hi, general, mes_sorting_command
...
def callback2(b_list):
    ...
    if len(b_list.order.bricks)>0:
        ordersRed = b_list.order.bricks[0].count #get the number of red bricks
        ordersBlue = b_list.order.bricks[1].count
        ordersYellow = b_list.order.bricks[2].count
...
def list_update():
		rospy.Subscriber("/mes/command", mes_sorting_command, callback2)
